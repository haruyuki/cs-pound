...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!link.includes('chickensmoothie.com') && !link.includes('chickencdn.com')) {
            await interaction.reply('There was an error parsing the link you provided, are you sure you provided a valid link?');
            return;
        }
        ...
        if (link.includes('item')) {
            ...
            if (itemRID === 0) {
                ...
                item = await ItemDB.findOne({ where: { itemLID: itemLID, itemRID: itemRID } });
            }

            if (item === null) {
                await interaction.reply('There is no data for this item yet :frowning:\nPlease note that current year items don\'t have data yet.');
                return;
            }
            ...
            if (link.includes('trans')) {
                await interaction.reply('Pets with items are unable to be identified :frowning:');
                return;
            ...
            if (exceptions.includes(petID)) {
                await interaction.reply('That pet is not identifiable at this growth stage :frowning:');
                return;
            }
            ...
            if (pet === null) {
                await interaction.reply('There is no data for this pet yet :frowning:\nPlease note that current year pets don\'t have data yet.');
                return;
            }
