...
function formatter([hour, minute, second]) {
    return [
        hour ? `${hour} hour${hour > 1 ? "s" : ""}${minute || second ? ", " : ""}` : "",
        minute ? `${minute} minute${minute > 1 ? "s" : ""}${second ? " and " : ""}` : "",
        second ? `${(hour || minute) ? "and " : ""}${second} second${second > 1 ? "s" : ""}` : ""
    ].join('');
}
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('time')
                .setDescription('The amount of time to wait before reminding you (e.g., 10s, 5m, 1h, 1h5m10s)')
                .setRequired(true)),
    async execute(interaction) {
        ...
        if (data.every(value => value === 0)) {
            return await interaction.reply({ content: "Invalid time format. Please provide a valid time format (e.g., 10s, 5m, 1h, 1h5m10s).", ephemeral: true });
        }

        const total = formatter(data);
        await interaction.reply({ content: `A reminder has been set for you in ${total}.`, ephemeral: true });

        setTimeout(() => {
            interaction.followUp({ content: `${interaction.user}, this is your ${amount} reminder!` });
        }, milliseconds);
    },
};